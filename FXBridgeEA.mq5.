...
{
   string input = IntegerToString(account) + EA_SECRET;
   uint h1 = 2166136261;
   for(int i = 0; i < StringLen(input); i++)
   {
      h1 ^= (uint)StringGetCharacter(input, i);
      h1 *= 16777619;
   }
   uint h2 = 19088743;
   for(int i = StringLen(input) - 1; i >= 0; i--)
   {
      h2 ^= (uint)StringGetCharacter(input, i);
      h2 *= 2246822519;
   }
